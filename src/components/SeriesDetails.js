...
const SeriesDetails = () => {
  ...
  return (
    <div className="max-w-full mx-auto p-4 sm:p-8 text-gray-800">
      <Link to="/Series" className="inline-block mb-4 text-red-500 hover:text-red-600">Back to movies</Link>
        ...
        <div className="flex flex-wrap -m-2">
          {reviews.map((review, index) => (
            <div key={index} className="w-full sm:w-1/2 p-2">
              <div className="bg-gray-100 p-4 rounded-lg shadow-lg flex items-start">
                <img src={
                ...
                <div>
                  <div className="font-bold text-lg mb-2 text-gray-500">{review.author}</div>
                  ...
                  {review.content.length > 220 && (
                    <button onClick={() => handleExpandClick(index)} className="mt-2 text-red-500 hover:text-red-600">{expandedReviews[index] ? 'Show less' : 'Show more'}</button>
                  )}
                </div>
              </div>
            </div>
